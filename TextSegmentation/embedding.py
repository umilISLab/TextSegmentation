...
from .utils import sentence_embeddings, batch_generator, additional_features
...
class SyntacticEmbedder:
    ...
    def word_feat_extraction(self, s: str):
        ...
        word_feats = [
            [
                dict(**word.morph.to_dict(), Ent=word.ent_type_)
                for word in sent
                if word.morph
            ]
            for sent in docs
    ...
    def __call__(self, corpus: Iterable[str], features: Iterable[str]):
        ...
        for feat in tqdm(features):
            ...
            feat_lists = map(
                lambda L: list(filter(lambda s: s in feat_modes, L)), feat_lists
            )
        ...
        print(
            f"[Syntactic Embedder] Found a total of {len(all_modes)} modes for features {features}"
        )

        feat_df = pd.DataFrame(feat_counts)

        synt_dicts = feat_df.apply(
            lambda row: {k: v for d in row for k, v in d.items()}, axis=1
        )
...
class TransformerEmbedder:
    ...
    def __call__(
        self, corpus: Iterable[str], return_dicts: bool = False, batch_size: int = 256
    ):
        ...
        if return_dicts:
            output = list(
                map(lambda a: {"B_" + str(i): x_i for i, x_i in enumerate(a)}, X_sbert)
            )
        else:
            output = X_sbert
...
class TfidfEmbedder:
    ...
    def __call__(self, corpus: Iterable[str], return_dicts: bool = False):
        ...
        if return_dicts:
            output = list(
                map(lambda a: {s: x_i for s, x_i in zip(tfidf_features, a)}, X_tfidf)
            )
        else:
            output = X_tfidf
...
def get_embeddings(
    ...
    if "synt" in all_features:
        ...
        print(
            f"[Embedding] Computing syntactic vectors with features {synt_features}, saving in {synt_path}."
        )
        try:
            assert try_loading
            ...
            print(
                "\tUnable to load syntactic embeddings from memory, computing them..."
            )
            syntactic_embedder = SyntacticEmbedder(spacy_model)
            synt_dicts = syntactic_embedder(corpus, features=synt_features)
    ...
    if "sbert" in all_features:
        ...
        print(
            f"[Embedding] Computing SBERT vectors with model {name} and pooling {pooling_flag}, saving in {sbert_path}."
        )
        try:
            assert try_loading
            ...
            sbert_embedder = TransformerEmbedder(model_name=sbert, pooling=pooling)
            sbert_dicts = sbert_embedder(corpus, return_dicts=True)
    ...
    if "tfidf" in all_features:
        ...
        print(
            f"[Embedding] Computing TFIDF vectors with dimension {tfidf_dim}, saving in {tfidf_path}."
        )
        try:
            assert try_loading
            ...
            tfidf_embedder = TfidfEmbedder(
                lang="italian", max_features=tfidf_dim, ngram_range=(1, 3)
            )
            tfidf_dicts = tfidf_embedder(corpus, return_dicts=True)
