...
def split_sentences(corpus, merge_short: int = 0):
    ...
    sent_df["sent_text"] = sent_df.apply(
    ...
    if merge_short:
        ...
        for i, row in sent_df.iterrows():
            if row["end"] - row["start"] <= merge_short and flag_prev:
                ...
                sent_df.loc[prev, "sent_text"] = " ".join(
                    [sent_df.loc[prev, "sent_text"], sent_df.loc[i, "sent_text"]]
                )
...
def exclude_partially_labeled(
    ...
    to_drop = [
        idx
        for idx, labelset in labelsets.items()
        if len(labelset) <= min_labels and null_label in labelset
    ]
...
def data_preprocessing(
    ...
    data["N_references"] = data[textfield].apply(
        lambda s: len(re.findall(r"(?<=([Cc]ass|[Aa]rtt?)\.?) *\d+", s))
    )
    ...
    data["Y"] = [
        v for v in mlb.fit_transform(data[labelfield].apply(lambda s: s.split(", ")))
    ]
    ...
    if synt_vectors:
        ...
        data["Synt"] = data.apply(
            lambda row: dict(
                sent_position=row["sent_position"],
                length=row["length"],
                refs=row["N_references"],
                **row["Synt"]
            ),
            axis=1,
        )
    ...
    data["feature_dicts"] = data.apply(
        lambda row: {k: v for feat in features for k, v in row[feat].items()}, axis=1
    )
    ...
    if null_label:
        ...
        to_remove = [
            id_
            for id_, count in doc_label_counts.items()
            if count[null_label] == max(count.values())
        ]
...
def sentence_embeddings(
    sentences, tokenizer, model, device, max_length=512, pooling: bool = True
):
    # Tokenize sentences
...
def doc_fold_mapping(data, idfield, labelfield, n_folds):
    ...
    strat_variable = [
        data.loc[data[idfield] == idx, labelfield].value_counts()[most_frequent_label]
        for idx in doc_ids
    ]
    ...
    folds = {
        doc_ids[idx]: i
        for i, (train, test) in enumerate(kfold_splitter.split(doc_ids, stratification))
        for idx in test
    }
    return folds
...
def pad_sequence_2d(tensors):
    ...
    padded_tensors = [
        torch.nn.functional.pad(t, (0, m[1], 0, m[0]), "constant", 0)
...
def map_prob_labels(
    Y: Union[np.ndarray, torch.Tensor], labels: Iterable[Union[str, int]], dim: int = -1
):
    """
...
def train_break_rules(losses: List[float], tol: float, break_seq: int):
    ...
    latest_deltas = [
        (l1 - l2) / l1 for l1, l2 in zip(losses[-break_seq - 1 :], losses[-break_seq:])
    ]
...
def extract_features_from_nodes(Gs):
    X = list(
        map(
            lambda G: torch.from_numpy(
                np.array([list(G.nodes[n].values()) for n in G.nodes])
            ),
            Gs,
        )
    )
    ...
    if priorposinfo:
        priors = list(
            map(
                lambda T: torch.cat(
                    [
                        onehot_tensor(len(classes), priorposinfo[0]).reshape((1, -1)),
                        T[1:-1, :],
                        onehot_tensor(len(classes), priorposinfo[-1]).reshape((1, -1)),
                    ],
                    dim=0,
                ),
                priors,
